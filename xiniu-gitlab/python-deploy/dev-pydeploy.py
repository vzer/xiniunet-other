...
import datetime
import subprocess
from config import PreServiceIp as ServiceIp
from config import PreWebIp as WebIp
from config import PreJobIp as JobIp
from config import user,password
from config import preserviceLocalPath as serviceLocalPath
from config import preserviceScriptFile as serviceScriptFile
from config import prewebScriptFile as webScriptFile
from config import prewebLocalPath as webLocalPath
from config import prejobLocalPath as jobLocalPath
from config import prejobScriptFile as jobScriptFile
                ...
                while True:
                    ...
                    if choice1.isdigit()and (int(choice1)>=0 and int(choice1)<len(value)):
                        ...
                        if putFileStatus==True:
                            try:
                                subprocess.check_call('ssh %s@%s sh %s/%s %s %s'%(user,value[int(choice1)],serviceLocalPath,serviceScriptFile,key,version),shell=True)
            ...
            for key,value in WebIp[int(choice)].items():
                while True:
                    ...
                    if choice1.isdigit()and (int(choice1)>=0 and int(choice1)<len(value)):
                        ...
                        if putFileStatus==True:
                            try:
                                subprocess.check_call('ssh %s@%s sh %s/%s %s %s'%(user,value[int(choice1)],webLocalPath,webScriptFile,key,version),shell=True)
                            except subprocess.CalledProcessError,msg:
                                printColor('red',msg)
                                printColor("red","User name or password is incorrect, please check!!")
                        choice3=rawInputColor("yellow","do you want need deploy the next %s host?(yes/no/quit):"%key)
            ...
            for key,value in JobIp[int(choice)].items():
                while True:
                    ...
                    if choice1.isdigit()and (int(choice1)>=0 and int(choice1)<len(value)):
                        ...
                        if putFileStatus==True:
                            try:
                                subprocess.check_call('ssh %s@%s sh %s/%s %s %s'%(user,value[int(choice1)],jobLocalPath,jobScriptFile,key,version),shell=True)
                            except subprocess.CalledProcessError,msg:
                                printColor('red',msg)
                                printColor("red","User name or password is incorrect, please check!!")
                        choice3=rawInputColor("yellow","do you want need deploy the next %s host?(yes/no/quit):"%key)
